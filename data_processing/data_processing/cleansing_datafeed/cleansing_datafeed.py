...
class Cleanser:
    ...
    def merged_product_by_size(self, input_list_articles):
        ...
        """
        headers = self.columns  # input_list_articles[0]
...
def cleansing():
    ...
    with Pool() as p:
        ...
        write_2_file(list_articles, "merged.tsv")
        ...
        # list_articles = list(tqdm.tqdm(p.map(cleanser.article_cleansing, list_articles),
        #                              total=len(list_articles)))
        ...
        cleansed_prices = [cleanser.clean_price(article) for article in
                           cleansed_fashion_suitable_for]  # list(tqdm.tqdm(p.map(cleanser.clean_price,
