...
from data_processing.data_processing.cleansing_datafeed.utils import convert_to_utf
...
from data_processing.data_processing.utils.utils import download_data_feeds_directory_path, \
    ...
    create_mapping_between_2_columns, get_lines_csv, write_2_file
...
def change_program_id_2_merchant_name(csv_file, shop_id_name_mapping_csv, ):
    ...
    with open(csv_file, encoding=enc) as input_csv:
        csv_reader2 = csv.reader(input_csv, delimiter=",", quotechar='"')
        with open(csv_file + "shop_id_2_name.csv", 'w', encoding="utf-8") as o:
            csv_writer = csv.writer(o, delimiter=",", quotechar='"')
            c = 0
            for row in csv_reader2:
                if c == 0:
                    for i in range(len(row)):
                        for key, value in shop_id2_name.items():
                            if key == row[i]:
                                row[pos_merchant_name] = value
                csv_writer.writerow(row)

    print("id ", pos_merchant_id)
...
def merging():
    ...
    list_articles = convert_to_utf(list_articles)
    write_2_file(list_articles, merged_data_feed_path)
